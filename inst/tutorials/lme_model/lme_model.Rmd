...
With that in mind, our model can be expressed as $Y_{i} = X_{i}'\beta + Z_{i}b_i + \epsilon_{ij}$ where
...
$b_i$ is vector of the random-effects regression coefficients. It is treated as a random variable.

$\epsilon_{ij}$ - is a vector of errors
...
$E(b_i|Y) = GZ'\Sigma_{i}^{-1}(Y_i-Xi\hat{\beta})$, where $\Sigma_{i} = Cov(Y_i) = Z_IGZ_i' + R_i$
