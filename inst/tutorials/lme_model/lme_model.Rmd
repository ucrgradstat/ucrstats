...
When working with multiple continuos variables, we can represent the variation with a square n x n matrix (where n is the number of explanatory variables). Each element in the matrix represents a covariance pair. The diagonal contains the variances of each variable (recall that $Covar(X,X) =Var(X)$). 

We will denote the covariance matrix as $\Sigma_i$.
...
$Cov(Y_i) = Z_iGZ_i' + R_i \sigma^2I_n$
